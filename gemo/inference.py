...
By default this uses the checkpoint and label map bundled with the
installed package (`gemo/checkpoints/best.pt`, `gemo/checkpoints/label_map.json`).
Pass `--checkpoint` / `--label-map` explicitly to use a different,
custom-trained model instead:

    gemo-infer --trk-path wholebrain.trk --checkpoint my_run/best.pt \\
        --label-map my_run/label_map.json --output-dir classified_output
"""
