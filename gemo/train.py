...
train
=====
Training entry point for the dual-branch `StreamlineClassifier`.

Usage
-----
    python train.py --trk-dir trk_dir --features-h5 features.h5 --bounds-h5 bounds.h5

Every other hyperparameter has a sane default in `config.py`; pass
`--help` to see the overridable subset, or edit `config.py` directly for
full control.

What this does
---------------
1. Derives the 30-class label map from tract names in `trk_dir`'s
   filenames (`streamline_model.build_label_mapping`).
2. Computes handcrafted-feature normalization statistics from
   `features.h5` in one streaming pass (`utils.compute_feature_normalization_stats`).
3. Splits subjects into train/validation (`dataset.build_datasets`).
4. Trains with `nn.CrossEntropyLoss` + Adam + StepLR, checkpointing
   `last.pt` every epoch (for resume) and `best.pt` whenever validation
   accuracy improves.
"""
...
def run_epoch(
    ...
    """
    Run one epoch of training (`optimizer` given) or evaluation
    (`optimizer=None`), returning `(avg_loss, avg_accuracy)`.
    """
...
def main(cfg: Config) -> None:
    ...
    logger.info("Scanning .trk headers for dataset composition (subjects, tracts, streamlines per class) ...")
    n_subjects, class_stats = compute_class_statistics(cfg.data.trk_dir, label_map, separator=cfg.data.separator)
    log_dataset_summary(n_subjects, label_map, class_stats)
    ...
    train_loader = DataLoader(
        ...
        drop_last=True,  # BatchNorm (in train mode) requires batch size > 1
    )
    val_loader = DataLoader(
        ...
        num_workers=cfg.train.num_workers,
        pin_memory=(device.type == "cuda"),
        # No drop_last here: eval-mode BatchNorm uses running stats, so a
        # trailing batch of any size (down to 1) is fine, and dropping
        # validation samples would understate accuracy on small datasets.
    )
    ...
    if cfg.train.class_weight_scheme != "none":
        class_weights = compute_class_weights(label_map, class_stats, scheme=cfg.train.class_weight_scheme).to(device)
        logger.info("Class weights (%s, ordered by class index): %s", cfg.train.class_weight_scheme, class_weights.tolist())
    ...
    for epoch in range(start_epoch, cfg.train.num_epochs):
        ...
        # Reshuffle subject order (and each subject's tract-file
        # round-robin order) for this epoch -- without this, the
        # training stream would repeat the exact same class ordering
        # every epoch, which is what caused the non-IID-batch training
        # instability seen previously (train accuracy spiking then
        # collapsing within an epoch, validation stuck near chance).
...
def _parse_args() -> Config:
    ...
    parser.add_argument(
        "--max-streamlines-per-class-per-epoch",
        type=int,
        default=None,
        help="Cap on streamlines drawn per (subject, tract-file) per epoch (0 disables capping). "
        f"Default: {DataConfig().max_streamlines_per_class_per_epoch}.",
    )
    parser.add_argument(
        "--class-weight-scheme",
        choices=["inverse_sqrt", "inverse", "none"],
        default=None,
        help="Class-imbalance loss weighting scheme. Default: inverse_sqrt.",
    )
    ...
    if args.max_streamlines_per_class_per_epoch is not None:
        cfg.data.max_streamlines_per_class_per_epoch = args.max_streamlines_per_class_per_epoch or None
    if args.class_weight_scheme is not None:
        cfg.train.class_weight_scheme = args.class_weight_scheme
    return cfg


def cli() -> None:
    """Entry point for the `gemo-train` console script (see pyproject.toml)."""
    main(_parse_args())
